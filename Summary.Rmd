...
2. Linear regression models A,B and C were created using R command ***lm(formula,...)***; model D is obtained from model B using R command ***step(lm,...)***. The final model D is :  
$`r LM3.equation_string`$  
 
\pagebreak
 Results of the models residuals analysis are shown in the table below:    

Model| SSE | MSE | Residuals range | R-squared | Adjusted R-squared
-----|-----|-----|-----------------|-----------|--------------------
 A   |`r LM0.SSE`|`r LM0.MSE`|`r min(LM0.resid)` ~ `r max(LM0.resid)`|`r LM0.Rs`|`r LM0.adjRs`       
 B   |`r LM1.SSE`|`r LM1.MSE`|`r min(LM1.resid)` ~ `r max(LM1.resid)`|`r LM1.Rs`|`r LM1.adjRs`       
 C   |`r LM2.SSE`|`r LM2.MSE`|`r min(LM2.resid)` ~ `r max(LM2.resid)`|`r LM2.Rs`|`r LM2.adjRs`       
 D   |`r LM3.SSE`|`r LM3.MSE`|`r min(LM3.resid)` ~ `r max(LM3.resid)`|`r LM3.Rs`|`r LM3.adjRs`       

3. From the table above, Fig.5 and Fig.6 we can see:
...
6. Summary of the best model D:
 ...
 + Residuals are not normally distributed (see Fig.8 and Fig.9);
